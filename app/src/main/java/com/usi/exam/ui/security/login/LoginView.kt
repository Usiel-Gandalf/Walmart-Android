...
import com.usi.exam.ui.components.TopAppBarWithoutIcon
        ...
        topBar = {
            ...
            Column(
                ...
                BodyLogin(
                    ...
                    onClicTryLogin = {
                        loginViewModel.loginUser(loginViewModel.email.value, loginViewModel.password.value)
                        if (isLoggedIn){
                            navigateToMain()
                        }else{
                            Toast.makeText(
                    ...
                    checkIfPhoneItsAbleToBiometricsAuthenticator = {
                        ...
                        if (loginViewModel.biometricEnabled.value) {
                            ...
                            Toast.makeText(
                                context,
                                "El dispositivo no admite datos biométricos",
                                Toast.LENGTH_LONG
                            ).show()
                        }
                    }
    ...
    LaunchedEffect(authenticationState) {
        when (authenticationState) {
            ...
            is AuthenticationState.Error -> {
                Toast.makeText(
                    context,
                    (authenticationState as AuthenticationState.Error).message,
                    Toast.LENGTH_LONG
                )
                    .show()
            }
